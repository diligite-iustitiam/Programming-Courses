...
namespace Programming_CoursesServer.Controllers
{
    ...
    public class CourseController : ControllerBase
    {
        ...
        public CourseController(ICoursesRepository courseRepository)
        {
        ...
        {
            ProgrammingCourse? c = await _repo.RetrieveAsync(id);
            if (c == null)
            {
        ...
        public async Task<IActionResult> Create([FromBody] ProgrammingCourse c)
        {
            ...
            ProgrammingCourse? addedCustomer = await _repo.CreateAsync(c);
          ...
          int id, [FromBody] ProgrammingCourse c)
        {
            ...
            ProgrammingCourse? existing = await _repo.RetrieveAsync(id);
            if (existing == null)
            {
                return NotFound(); // 404 Resource not found
            }
